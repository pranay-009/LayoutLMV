...
def loss_fn(pred,target):
    return nn.CrossEntropyLoss()(pred.view(-1,3),target.view(-1))


class ModelModule(nn.Module):
    def __init__(self,n_classes:int) -> None:
        super().__init__()
        self.model = LayoutLMv3ForTokenClassification.from_pretrained('D:\DocLayout\LayoutLMV3_Fine_Tuning\input\layoutlmv3-base')
